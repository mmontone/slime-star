...
;;;
(defvar *traced-specs* '())
(defvar *before-trace-hooks* '())
(defvar *after-trace-hooks* '()
  "Functions that get called when a traced function returns.
They are called with a trace-entry object.")

(defslimefun dialog-trace (spec)
  (flet ((before-hook (args)
           (let ((trace (make-instance 'trace-entry
                                       :spec      spec
                                       :args      args
                                       :parent    (current-trace))))
             (setf (current-trace) trace)
             (dolist (hook *before-trace-hooks*)
               (funcall hook trace))))
         (after-hook (retlist)
